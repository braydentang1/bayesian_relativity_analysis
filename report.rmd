...
$$X_{i} \big|N_{i}, \phi, \mu_{i} \sim \text{Inverse Gaussian}\left(\mu_{i}, \frac{N_{i}}{\phi}\right),$$

$$\mu_{i} \big| \beta, \vec{b} = exp(\beta + b_{ij} + b_{ik} + b_{il} + b_{im}),$$
$$\beta \sim \text{Normal}(0, 7),$$
$$\vec{b} \big| \sigma^2 \sim \text{MVN}(\vec{0}, \sigma^2),$$
$$\sigma^2 = \begin{bmatrix}\sigma_j^2 & 0 & 0 & 0\\0 & \sigma_{k}^2\ & 0 & 0\\ 0 & 0 & \sigma_{l}^2 & 0 \\ 0 & 0 & 0 \ & \sigma_{m}^2 \end{bmatrix},$$
$$N_{i} \big|\xi_{i}, \nu \sim \text{Negative Binomial}(\xi_{i}, \nu),$$
$$\xi_{i} \big|\psi, \vec{t} = exp(\psi + t_{ij} + t_{ik} + t_{il} + t_{im} + log(\text{exposure}_{i})),$$
$$\vec{t} \big| \tau^2 \sim \text{MVN}(\vec{0}, \tau^2),$$
$$\tau^2 = \begin{bmatrix}\tau_j^2 & 0 & 0 & 0\\0 & \tau_{k}^2\ & 0 & 0\\ 0 & 0 & \tau_{l}^2 & 0 \\ 0 & 0 & 0 \ & \tau_{m}^2 \end{bmatrix},$$
$$\psi \sim Normal(0, 6),$$
$$\phi \sim \text{Half-Cauchy}(0, 8),$$
