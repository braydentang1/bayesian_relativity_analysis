...
One solution to the above problem is to group the sparse level of
...
I don't really have strong apriori beliefs regarding the effects (I really have no context regarding the data)
but I still want enough regularization to shrink effects to 0, if it is justifiable.

For the variance parameter, I set $\alpha$ to 8. This is again a weakly informative prior.
...
First, I generate samples from the posterior predictive distribution. 
I then perform the posterior predictive check (i.e. compare the generated data against the observed data).
...
weighting depends on the variation **between** rating levels,
...
influences the amount of variation within a level to control pooling instead, due to the use of weighting in the models to address issues with data aggregation). This is often interpreted as a way to induce the sharing of 
...
$$ln(\mu_{i_{[jklm]}}) \ \big| \ \beta, \sigma_{[jklm]}^2 \sim \text{Normal}(\beta, \sigma_{[jklm]}^2),$$
$$\beta \sim \text{Normal}(0, 7),$$
$$\sigma^2_{[jklm]} \sim \text{Half Student's T(3, 0, 2.5)}.$$
...
The frequency model for $N_{i_{[jklm]}}$ follows similarly to the above formulation for severity.
...
As mentioned previously, a base class (the class where all other rating combinations are compared against) is required. In the data, this corresponds to the rating combination corresponding to row `r which(data$vehicle_exposure_years == max(data$vehicle_exposure_years))` (1 = km, 4 = zone, 7 = bonus, 9 = make).

We can now calculate posterior distributions for the relativities for each rating variable.
...
create_tables <- function(rels, rv_name) {
	...
	table <- tibble(
		temp = 1:ncol(rels),
		`Estimated Relativity (median)` = apply(X = rels, MARGIN = 2, FUN = function(x) median(x)),
		`Lower 90 Credible Interval` = apply(X = rels, MARGIN = 2, FUN = function(x) quantile(x, 0.05)),
		`Upper 90 Credible Interval` = apply(X = rels, MARGIN = 2, FUN = function(x) quantile(x, 0.95)),
		`Standard Deviation` = apply(X = rels, MARGIN = 2, FUN = function(x) sd(x))
	)
...
In this case, we can see that the estimated relativities (the median of each posterior predictive distribution) follows a monotonic pattern and likely no adjustment would be used here. 

However, the point is to emphasize that while our single "best guess" might be 1, 1.231, 1.339, 1.458, and 1.717 
for a chosen set of relativities, values such as 1, 1.3, 1.6, 1.8, 2 could have reasonably been observed as well. Another set could have been 1, 0.97, 1.4, 1.3, 1.5. These combinations aren't as likely, but plausible given the data.

In addition, notice that rating level 2 contains a relativity of 1 in its 90% credible interval. This implies that there is some probability that level 2 may actually be worse than the base class of level 1, even though our "best guess" would state it as higher. In fact, the posterior probability that the true relativity for level 2 is less than 1 is `r round(mean(km_rels[, 2] < 1), 3)`. Arguably, this is still not that likely (for comparison, the probabilities of the other levels having less than 1 relativity are 0.032, 0.012, and 0.001 for levels 3, 4, and 5, respectively) but it is not trivially 0 either. 

In addition, we can quantify essentially any hypothesis that we want freely. As an example, some questions that the user may be interested in:

- The exact probability that level 2 has a higher relativity than level 3 or 4 or 5:

```{r}
mean(apply(X = km_rels, MARGIN = 1, FUN = function(x) max(x)) == km_rels[, 2])
```
- The exact probability that level 3 exceeds the relativity of level 2 by at least 0.1 (or any arbitrary constant):

```{r}
mean(km_rels[, 3] - km_rels[, 2] > 0.1)
```
Combining these probabilities with the expected costs (if any) of making the "wrong" decision (utilizing business sense and how conservative or aggressive the company wishes to be) can be used to make decisions. 
