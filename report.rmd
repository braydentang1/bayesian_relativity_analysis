...
It looks like the current generative model gives too much density to small losses in general.
That is, the proposed gamma distribution does not have heavy enough tails.

There are many heavy tailed distributions that are worth looking at. 
...
The generative model is exactly the same as before, except now we replace $X_{i} \big | N_{i}, \phi, \eta_{i}$ with a log-normal that has parameters $R_{i}\beta, \frac{\sigma}{N_{i}}$.
...
data {
  ...
  limit since now the severity can technically be < 0*/
}
...
The fit is a bit mixed. There is clearly more mass being put towards the tail
and less towards small severity amounts, however the peak in the severity distribution is still not very plausible under the generative model and is arguably worse than the previous gamma model.

The pure premium model looks better in some areas and slightly worse than others as a result.
The region of highest density is being modeled a bit better than the gamma, but argubly worse than the gamma result at lower pure premium values.
