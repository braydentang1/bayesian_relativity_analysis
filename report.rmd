...
tibble(
	...
	datatable(
		...
		options = list(
			scrollY = 300, searching = TRUE, height = "400px", scrollX = 300)
		) %>%
...
Gelman-Rubin's Rhat tries to assess how much each chain differs in terms of their variance. Due to the law of total variance, the marginal variance of a parameter can be estimated as a weighted average of the average within chain variance and the variance of the averages between chains.
...
Gelman and Rubin recommend that Rhat values for **all** parameters are less than 1.1. As we can see here, the largest Rhat we observe is 1.0016, which is well below 1.1. This is good evidence to suggest that all of the separate Markov Chains we have run have converged to the same posterior distribution. Thus, there is good evidence to suggest that there are no convergence issues.
