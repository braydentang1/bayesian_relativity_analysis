...
$$b_{i_{[jklm]}} \sim \text{Normal}(0, \sigma_{i_{[jklm]}}^2),$$
...
$$t_{i_{[jklm]}} \sim \text{Normal}(0, \tau_{i_{[jklm]}}^2),$$
...
colnames(km_rels) <- paste("Level", 1:ncol(km_rels))
...
error_bars <- tibble(
	Level = paste("Level", 1:ncol(km_rels)),
	lower = apply(km_rels, MARGIN = 2, FUN = function(x) quantile(x, 0.05)),
	upper = apply(km_rels, MARGIN = 2, FUN = function(x) quantile(x, 0.95))
) %>%
...
ggplot(data = ., aes(x = Level, y = Relativity)) + 
	geom_violin() +
	theme_bw() +
	labs(title = "Kilometre Relativity Densities") +
	geom_errorbar(
		data = error_bars,
