...
posterior_pred_check <- function(posterior_predictive_samples, y, n, variable) {
	...
	ggplot(data_tibble, aes(x = var)) +
		...
		geom_density(data = tibble(actual = y), aes(x = actual), size = 1) +
		labs(x = variable)
...
simulate_pure_premium <- (claim_count_posterior_pred /
									exp(pg_posterior_samples$exposures)) * simulate_severity
...
simulate_pure_premium_nb <- (claim_count_posterior_pred_nb /
									exp(nb_posterior_samples$exposures)) * simulate_severity_nb
